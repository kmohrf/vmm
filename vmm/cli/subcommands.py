...
def alias_info(ctx):
    ...
    try:
        ...
        if err.code is ACCOUNT_EXISTS:
            w_err(0, ctx.plan_a_b % {"subcommand": "userinfo", "object": address})
            ctx.args.scmd = "userinfo"
            ctx.args.details = None
            user_info(ctx)
        elif err.code is RELOCATED_EXISTS:
            w_err(0, ctx.plan_a_b % {"subcommand": "relocatedinfo", "object": address})
...
def aliasdomain_add(ctx):
    ...
    ctx.hdlr.aliasdomain_add(ctx.args.fqdn.lower(), ctx.args.destination.lower())
...
def aliasdomain_info(ctx):
    ...
    try:
        ...
        if err.code is ALIASDOMAIN_ISDOMAIN:
            w_err(0, ctx.plan_a_b % {"subcommand": "domaininfo", "object": fqdn})
...
def aliasdomain_switch(ctx):
    ...
    ctx.hdlr.aliasdomain_switch(ctx.args.fqdn.lower(), ctx.args.destination.lower())
...
def domain_delete(ctx):
    ...
    ctx.hdlr.domain_delete(
        ctx.args.fqdn.lower(), ctx.args.delete_directory, ctx.args.force
    )
...
def domain_info(ctx):
    ...
    try:
        ...
        if err.code is DOMAIN_ALIAS_EXISTS:
            w_err(0, ctx.plan_a_b % {"subcommand": "aliasdomaininfo", "object": fqdn})
        ...
        if not details:
            ...
            info[0]["messages"] = locale.format("%d", info[0]["messages"], True)
...
def domain_quota(ctx):
    ...
    ctx.hdlr.domain_quotalimit(
        ctx.args.fqdn.lower(), ctx.args.storage, ctx.args.messages, ctx.args.force
    )
...
def domain_transport(ctx):
    ...
    ctx.hdlr.domain_transport(
        ctx.args.fqdn.lower(), ctx.args.transport.lower(), ctx.args.force
    )
...
def relocated_info(ctx):
    ...
    try:
        _print_relocated_info(addr=relocated, dest=ctx.hdlr.relocated_info(relocated))
    except VMMError as err:
        if err.code is ACCOUNT_EXISTS:
            w_err(0, ctx.plan_a_b % {"subcommand": "userinfo", "object": relocated})
            ctx.args.scmd = "userinfo"
            ctx.args.details = None
            user_info(ctx)
        elif err.code is ALIAS_EXISTS:
            w_err(0, ctx.plan_a_b % {"subcommand": "aliasinfo", "object": relocated})
...
def user_add(ctx):
    ...
    gen_pass = ctx.hdlr.user_add(
        ctx.args.address.lower(), ctx.args.password, ctx.args.note
    )
...
def user_delete(ctx):
    ...
    ctx.hdlr.user_delete(
        ctx.args.address.lower(), ctx.args.delete_directory, ctx.args.force
    )
...
def user_info(ctx):
    ...
    try:
        ...
        if err.code is ALIAS_EXISTS:
            w_err(0, ctx.plan_a_b % {"subcommand": "aliasinfo", "object": address})
            ctx.args.scmd = "aliasinfo"
            alias_info(ctx)
        elif err.code is RELOCATED_EXISTS:
            w_err(0, ctx.plan_a_b % {"subcommand": "relocatedinfo", "object": address})
        ...
        if ctx.args.details in (None, "du"):
            info["quota storage"] = _format_quota_usage(
                info["ql_bytes"], info["uq_bytes"], True, info["ql_domaindefault"]
            )
            info["quota messages"] = _format_quota_usage(
                info["ql_messages"],
                info["uq_messages"],
            ...
            info[0]["quota storage"] = _format_quota_usage(
                info[0]["ql_bytes"],
                info[0]["uq_bytes"],
                True,
                info[0]["ql_domaindefault"],
            )
...
def user_password(ctx):
    ...
    if ctx.args.pwhash:
        ...
        ctx.hdlr.user_password(
            ctx.args.address.lower(), ctx.args.password, ctx.args.scheme
        )
...
def user_quota(ctx):
    ...
    ctx.hdlr.user_quotalimit(
        ctx.args.address.lower(), ctx.args.storage, ctx.args.messages
    )
...
def setup_parser():
    ...
    class ArgParser(ArgumentParser):
        ...
        def _get_formatter(self):
            return self.formatter_class(
                prog=self.prog, width=WS_ROWS, max_help_position=26
            )
    ...
    epilog = _(
        'use "%(prog)s <subcommand> -h" for information about the ' "given subcommand"
    )
    ...
    parser.add_argument(
        ...
        nargs=0,
        help=_("show %(prog)s's version and copyright " "information and exit"),
    )
    subparsers = parser.add_subparsers(
        metavar=_("<subcommand>"), title=_("list of available subcommands")
    )
    ...
    cg = a(
        ...
        epilog=_(
            "This subcommand is used to display the actual value of "
    ...
    ll.add_argument(
        "-p", help=_("the pattern to search for"), metavar="PATTERN", dest="pattern"
    )
    ...
    lr = a(
        "listrelocated",
        aliases=("lr",),
        help=_("list all relocated users or search for relocated users by " "pattern"),
        epilog=fill(
            _(
    ...
    lp = a(
        "listpwschemes",
        aliases=("lp",),
        help=_("lists all usable password schemes and password encoding " "suffixes"),
        epilog=fill(
            _(
    ...
    da = a(
        "domainadd",
        aliases=("da",),
        help=_("create a new domain"),
        epilog=fill(
            _(
                "Adds the new domain into the database and creates "
    ...
    da.add_argument(
        "-n", metavar="NOTE", dest="note", help=_("the note that should be set")
    )
    da.add_argument(
        "-t",
        metavar="TRANSPORT",
        dest="transport",
        help=_("a Postfix transport (transport: or " "transport:nexthop)"),
    )
    da.set_defaults(func=domain_add, scmd="domainadd")

    details = ("accounts", "aliasdomains", "aliases", "catchall", "relocated", "full")
    di = a(
        "domaininfo",
        aliases=("di",),
    ...
    do = a(
        ...
        epilog=_(
            "With this subcommand, it is possible to attach a note to "
    ...
    do_grp.add_argument(
        "-d", action="store_true", dest="delete", help=_("delete the note, if any")
    )
    do_grp.add_argument(
        "-n", metavar="NOTE", dest="note", help=_("the note that should be set")
    )
    ...
    dq.add_argument(
        "storage",
        type=quota_storage,
        help=_("quota limit in {kilo,mega,giga}bytes e.g. 2G " "or 2048M",),
    )
    dq.add_argument(
        "-m",
        default=0,
    ...
    dq.add_argument(
        "--force", action="store_true", help=_("enforce the limit for all accounts")
    )
    ...
    ds = a(
        ...
        aliases=("ds",),
        help=_(
            "enables the specified services and disables all not "
            "specified services of the given domain"
        ),
        epilog=fill(
            _(
                "To define which services could be used by the users "
    ...
    dt = a(
        ...
        epilog=fill(
            _(
                "A new transport for the indicated domain can be set "
    ...
    dt.add_argument(
        "transport", help=_("a Postfix transport (transport: or " "transport:nexthop)")
    )
    dt.add_argument(
        "--force", action="store_true", help=_("enforce the transport for all accounts")
    )
    ...
    dd.add_argument(
        "--force",
        action="store_true",
        help=_("also delete all accounts, aliases and/or " "relocated users"),
    )
    ...
    ada = a(
        ...
        epilog=_(
            "This subcommand adds the new alias domain (fqdn) to "
            "the destination domain that should be aliased."
        ),
    )
    ada.add_argument("fqdn", help=_("a fully qualified domain name"))
    ada.add_argument("destination", help=_("the fqdn of the destination domain"))
    ...
    ads = a(
        ...
        epilog=_(
            "If the destination of the existing alias domain fqdn "
            "should be switched to another destination use this "
            "subcommand."
        ),
    )
    ads.add_argument("fqdn", help=_("a fully qualified domain name"))
    ads.add_argument("destination", help=_("the fqdn of the destination domain"))
    ...
    add = a(
        ...
        epilog=_("Use this subcommand if the alias domain fqdn should be " "removed."),
    )
    ...
    ua = a(
        ...
        epilog=fill(
            _(
                "Use this subcommand to create a new e-mail account "
    ...
    ua.add_argument("address", help=_("an account's e-mail address (local-part@fqdn)"))
    ua.add_argument(
        "-n", metavar="NOTE", dest="note", help=_("the note that should be set")
    )
    ua.add_argument(
        "-p", metavar="PASSWORD", dest="password", help=_("the new user's password")
    )
    ...
    ui = a(
        ...
        epilog=fill(
            _(
                "This subcommand displays some information about "
    ...
    ui.add_argument("address", help=_("an account's e-mail address (local-part@fqdn)"))
    ui.add_argument(
        "-d",
        choices=details,
        metavar="DETAILS",
    ...
    un.add_argument("address", help=_("an account's e-mail address (local-part@fqdn)"))
    ...
    un_grp.add_argument("-n", help=_("a user's real name"), metavar="NAME", dest="name")
    ...
    uo.add_argument("address", help=_("an account's e-mail address (local-part@fqdn)"))
    ...
    up.add_argument("address", help=_("an account's e-mail address (local-part@fqdn)"))
    up_grp = up.add_mutually_exclusive_group()
    up_grp.add_argument(
        "-p", metavar="PASSWORD", dest="password", help=_("the user's new password")
    )
    up.add_argument(
        "-s",
        metavar="SCHEME",
    ...
    up_grp.add_argument(
        ...
        metavar="PWHASH",
        dest="pwhash",
        help=_("set the given password hash as-is as new " "password"),
    )
    ...
    uq = a(
        ...
        epilog=fill(
            _(
                "This subcommand is used to set a new quota limit "
    ...
    uq.add_argument("address", help=_("an account's e-mail address (local-part@fqdn)"))
    uq.add_argument(
        "storage",
        type=quota_storage,
        help=_("quota limit in {kilo,mega,giga}bytes e.g. 2G " "or 2048M"),
    ...
    us = a(
        ...
        aliases=("us",),
        help=_(
            "enable the specified services and disables all not " "specified services"
        ),
        epilog=fill(
            _(
                "To grant a user access to the specified service(s), "
    ...
    us.add_argument("address", help=_("an account's e-mail address (local-part@fqdn)"))
    us.add_argument(
        "-s",
        choices=SERVICES + ("domain",),
        help=_("services which should be usable"),
        metavar="SERVICE",
        nargs="+",
        dest="services",
    ...
    ut.add_argument("address", help=_("an account's e-mail address (local-part@fqdn)"))
    ut.add_argument(
        "transport", help=_("a Postfix transport (transport: or " "transport:nexthop)")
    )
    ...
    ud.add_argument("address", help=_("an account's e-mail address (local-part@fqdn)"))
    ud.add_argument(
        "--delete-directory",
        action="store_true",
    ...
    ud.add_argument(
        "--force", action="store_true", help=_("also delete assigned alias addresses")
    )
    ...
    aa = a(
        "aliasadd",
        aliases=("aa",),
        help=_("create a new alias e-mail address with one or more " "destinations"),
        epilog=fill(
            _(
    ...
    aa.add_argument("address", help=_("an alias' e-mail address (local-part@fqdn)"))
    aa.add_argument(
        "destination",
        nargs="+",
    ...
    ai = a(
        ...
        epilog=_(
            "Information about the alias with the given address can "
            "be displayed with this subcommand."
        ),
    )
    ai.add_argument("address", help=_("an alias' e-mail address (local-part@fqdn)"))
    ...
    ad.add_argument("address", help=_("an alias' e-mail address (local-part@fqdn)"))
    ad.add_argument(
        "destination",
        nargs="*",
        help=_("a destination's e-mail address (local-part@fqdn)"),
    ...
    cai = a(
        "catchallinfo",
        aliases=("cai",),
        help=_("show the catch-all destination(s) of the specified " "domain"),
        epilog=_(
    ...
    cad = a(
        ...
        aliases=("cad",),
        help=_(
            "delete the specified catch-all destination or all of a "
            "domain's destinations"
        ),
        epilog=_(
            "With this subcommand, catch-all aliases defined for a "
    ...
    ra = a(
        ...
        epilog=_("A new relocated user can be created with this " "subcommand."),
    )
    ra.add_argument(
        "address", help=_("a relocated user's e-mail address " "(local-part@fqdn)")
    )
    ra.add_argument(
        "newaddress", help=_("e-mail address where the user can be reached now")
    )
    ...
    ri = a(
        ...
        epilog=_(
            "This subcommand shows the new address of the relocated "
    ...
    rd = a(
        ...
        epilog=_(
            "Use this subcommand in order to delete the relocated "
            "user with the given address."
        ),
    )
    rd.add_argument(
...
def _get_order(ctx):
    ...
    if ctx.args.scmd == "domaininfo":
        ...
        if ctx.args.details in ("du", "full") or ctx.cget("account.disk_usage"):
            order = (
                ("address", 0),
                ("name", 0),
                ("uid", 1),
...
def _format_quota_usage(limit, used, human=False, domaindefault=False):
    ...
    if limit:
        q_usage["percent"] = locale.format("%6.2f", 100.0 / limit * used, True)
    else:
        ...
        if title != _("alias domains"):
            ...
            for domain in alist:
                if not domain.startswith("xn--"):
                    ...
                    w_std("\t%s (%s)" % (domain, domain.encode("utf-8").decode("idna")))
        print()
    else:
        w_std(_("\tNone"), "")
...
def _print_catchall_info(domain, destinations):
    ...
    w_std(title, "-" * len(title))
    w_std(_("\tMail to unknown local-parts in domain %s will be sent to:") % domain)
    w_std(*("\t     * %s" % dest for dest in destinations))
    print()
...
def _print_address_list(which, dids, addresses, matching):
    ...
    try:
        ...
        raise VMMError(
            _("Invalid address type for list: '%s'") % which, INVALID_ARGUMENT
        )
    if addresses:
        if which & (which - 1) == 0:
